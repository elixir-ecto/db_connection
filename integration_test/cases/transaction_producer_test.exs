defmodule TransactionProducerTest do
  ...
  test "start_link produces result" do
    ...
    {:ok, stage} = P.start_producer(pool, %Q{}, [:param], opts)
  ...
  test "start_link with prepare: true produces result" do
    ...
    opts = [agent: agent, parent: self()]
    {:ok, pool} = P.start_link(opts)
    {:ok, stage} = P.start_producer(pool, %Q{}, [:param], [stage_prepare: true] ++ opts)
    mon = Process.monitor(stage)
    assert [{stage, [cancel: :transient]}] |> GenStage.stream() |> Enum.to_list() == [%R{}, %R{}]
  ...
  test "stage declare disconnects" do
    ...
    Process.flag(:trap_exit, true)
    {:ok, pool} = P.start_link(opts)
    assert {:error, {^err, _}} = P.start_producer(pool, %Q{}, [:param], opts)
  ...
  test "stage declare bad return raises and stops" do
    ...
    Process.flag(:trap_exit, true)
    {:error, {%DBConnection.ConnectionError{}, _} = reason} = P.start_producer(pool, %Q{}, [:param], opts)
