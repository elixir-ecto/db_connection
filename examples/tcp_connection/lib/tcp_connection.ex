defmodule TCPConnection do
  ...
  def handle_query({:send, data}, _, {sock, _} = state) do
  ...
  def handle_query({:recv, bytes, timeout}, _, {sock, <<>>} = state) do
  ...
  def handle_query({:recv, bytes, _}, _, {sock, buffer})
  ...
  def handle_query({:recv, bytes, timeout}, _, {sock, buffer} = state) do
